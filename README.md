...
memory_consolidate 도구가 실행되거나 서버 내부 스케줄러가 트리거할 때 동작하는 12단계 유지보수 파이프라인이다.
...
3. **만료 파편 삭제 (다차원 GC)**: 3가지 복합 조건으로 판정한다. (a) utility_score < 0.15 + 비활성 60일, (b) fact/decision 고립 파편(접근 0회, 링크 0개, 30일 경과, importance < 0.2), (c) 기존 하위 호환 조건(importance < 0.1, 90일). gracePeriod 7일 이내 파편은 보호된다. 1회 최대 50건 삭제. `is_anchor=true`, `permanent` 계층 제외
...
Gemini API 키는 기존 `GEMINI_API_KEY` 환경 변수를 재사용할 수 있다.
