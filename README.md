...
| ema_activation | FLOAT | DEFAULT 0.0 | ACT-R 기저 활성화 EMA 근사값. `incrementAccess()` 호출 시 `α * (Δt_sec)^{-0.5} + (1-α) * prev` 수식으로 갱신(α=0.3). `_computeRankScore()`에서 importance 부스트로 활용 |
...
15단계 유지보수 파이프라인을 수동으로 실행한다. 서버 내부에서 6시간 간격으로 자동 실행되지만 수동 호출도 가능하다. 파라미터 없음.
...
memory_consolidate 도구가 실행되거나 서버 내부 스케줄러(6시간 간격, CONSOLIDATE_INTERVAL_MS로 조정)가 트리거할 때 동작하는 15단계 유지보수 파이프라인이다.
...
> 차원 변경 시 주의: `EMBEDDING_DIMENSIONS`를 바꾸면 PostgreSQL 스키마도 변경해야 한다. `node lib/memory/migration-007-flexible-embedding-dims.js`와 `node lib/memory/backfill-embeddings.js`를 순서대로 실행할 것.
