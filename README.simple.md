...
PostgreSQL만 있으면 핵심이 돌아감. Redis 넣으면 L1 캐시 검색이 켜지고, Embedding API 넣으면 L3 시맨틱 검색 + 자동 링크가 켜지고, NLI 모델은 그냥 깔려있으니까 논리적 모순은 알아서 잡고, Gemini CLI까지 깔면 수치/도메인 모순 + 자동 reflect까지 풀가동됨. 하나씩 붙여도 되고 다 빼도 됨.
