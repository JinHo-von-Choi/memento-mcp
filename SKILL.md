...
#### LLM Provider 체인 확장 (v2.9.0)

`LLM_PRIMARY` 및 `LLM_FALLBACKS`에 `codex-cli`, `copilot-cli` 추가 지원.
...
Gemini CLI 외 다수의 외부 provider로 자동 fallback 가능. v2.9.0에서 `codex-cli`, `copilot-cli` 추가. 설정: `LLM_PRIMARY=gemini-cli` (기본) + `LLM_FALLBACKS` JSON 배열. env 미설정 시 기존 Gemini CLI 단독 동작 유지. 자세한 운영은 `docs/operations/llm-providers.md` 참조.
