 ...
 * 작성자: 최진호
 * 작성일: 2026-01-30
 * 수정일: 2026-04-06 (graceful shutdown 시 Redis 세션 보존 옵션 추가)
 */

import crypto            from "crypto";
import { SESSION_TTL_MS, REDIS_ENABLED, CACHE_SESSION_TTL, SSE_HEARTBEAT_INTERVAL_MS, SSE_MAX_HEARTBEAT_FAILURES } from "./config.js";
import {
...
export async function createStreamableSession(authenticated = false, keyId = null, groupKeyIds = null, permissions = null, defaultWorkspace = null, mode = null) {
  ...
  const session = {
    ...
    setSseResponse  : (res) => {
      ...
      if (res) {
        ...
        heartbeat         = setInterval(() => {
          ...
          /** Heartbeat failure handling with tolerance for transient issues.
           *
           *  - 1–3 consecutive failures: assume transient (browser throttle, backpressure)
           *    → set gracePeriod flag and continue sending pings
           *  - 4+ consecutive failures: likely a dead connection
           *    → close session to free resources
           *  - Any successful ping resets the counter to 0
           *
           *  The previous threshold of 3 was too aggressive for cloud-hosted clients
           *  (claude.ai, ChatGPT Web) whose browser tabs aggressively throttle SSE
    ...
    close: async ({ preserveRedis = false } = {}) => {
      ...
      // Redis 세션 삭제 (preserveRedis=true 시 유지 — TTL 자연 만료에 의존)
