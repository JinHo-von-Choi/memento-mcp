...
export class FragmentWriter {
  ...
  async ensureSchema() {
    ...
    try {
      ...
      console.warn(`[FragmentWriter] Schema check failed: ${err.message}`);
    }
  }
  ...
  async archiveVersion(fragment, agentId = "default") {
    await queryWithAgentVector(agentId,
      ...
      "write"
    ).catch(err => console.warn(`[FragmentWriter] archiveVersion failed: ${err.message}`));
  }
  ...
  async incrementAccess(ids, agentId = "default", { noEma = false } = {}) {
    ...
    if (noEma) {
      await queryWithAgentVector(agentId,
        ...
        [ids],
        "write"
      ).catch(err => console.warn(`[FragmentWriter] incrementAccess failed: ${err.message}`));
    } else {
      await queryWithAgentVector(agentId,
        `UPDATE ${SCHEMA}.fragments
               SET access_count      = access_count + 1,
  ...
  async touchLinked(retrievedIds, agentId) {
    ...
    await queryWithAgentVector(agentId,
      `UPDATE ${SCHEMA}.fragments
       ...
       WHERE id IN (
         SELECT DISTINCT
           CASE WHEN fl.from_id = ANY($1::uuid[]) THEN fl.to_id
                ELSE fl.from_id
           END
         FROM ${SCHEMA}.fragment_links fl
         WHERE (fl.from_id = ANY($1::uuid[]) OR fl.to_id = ANY($1::uuid[]))
