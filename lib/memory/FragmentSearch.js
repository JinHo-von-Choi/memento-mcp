...
export class FragmentSearch {
  ...
  async search(query) {
    ...
    /** timeRange 파싱 및 검증 */
    const timeRange = parseTimeRange(query.timeRange);
    ...
    if (query.text && EMBEDDING_ENABLED) {
      ...
      const [l2Results, l3Results] = await Promise.all([
        this._searchL2(query, l1MissIds, agentId, keyId, timeRange),
        this._searchL3(query, agentId, keyId, timeRange)
      ]);
      ...
      const l2Results  = await this._searchL2(query, l1MissIds, agentId, keyId, timeRange);
    ...
    const searchEvent = buildSearchEvent(
      ...
      clean,
      {
        ...
        keyId       : (Array.isArray(keyId) ? keyId[0] : keyId) ?? null,
        latencyMs   : _totalMs,
  ...
  async _searchL3(query, agentId = "default", keyId = null, timeRange = null) {
    try {
      ...
      return this.store.searchBySemantic(vec, limit || 10, minSimilarity || 0.2, agentId, keyId, query.includeSuperseded || false, timeRange);
    } catch (err) {
