...
export async function fetchGraphNeighbors(seedIds, maxTotal = 10, _agentId = "default", keyId = null) {
  ...
  /** keyId 정규화: 단일 값 또는 배열 모두 배열로 통일. null이면 마스터(필터 없음). */
  const effectiveKeyId = Array.isArray(keyId) ? keyId : (keyId != null ? [keyId] : null);

  let keyFilter = "";
  const params  = [seedIds, seedIds, maxTotal];

  if (effectiveKeyId !== null) {
    keyFilter = `AND (f.key_id IS NULL OR f.key_id = ANY($4::int[]))`;
