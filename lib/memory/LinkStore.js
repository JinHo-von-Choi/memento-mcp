...
export class LinkStore {
  ...
  async createLink(fromId, toId, relationType = "related", agentId = "default", weight = 1) {
     ...
     * queryWithAgentVector가 단일 트랜잭션이므로 lock + INSERT를 하나의 SQL로 묶는다.
    ...
    await queryWithAgentVector(agentId,
      `SELECT pg_advisory_xact_lock(hashtext($4::text)), pg_advisory_xact_lock(hashtext($5::text));
      ...
      [fromId, toId, relationType, lockFirst, lockSecond, weight],
      "write"
    );
