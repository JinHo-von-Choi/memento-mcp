...
export class FragmentStore {
  ...
  /**
     ...
     * @param {string|null} keyId - null: 마스터(전체), string: 해당 API 키 소유만
     */
  async searchBySemantic(queryEmbedding, limit = 10, minSimilarity = 0.3, agentId = "default", keyId = null, includeSuperseded = false) {
  ...
  async update(id, updates, agentId = "default", keyId = null, existing = null) {
    if (!existing) {
      existing = await this.getById(id, agentId);
  ...
  async createLink(fromId, toId, relationType = "related", agentId = "default") {
    return this.links.createLink(fromId, toId, relationType, agentId);
  }

  async getLinkedFragments(fromIds, relationType = null, agentId = "default", keyId = null) {
    return this.links.getLinkedFragments(fromIds, relationType, agentId, keyId);
  }

  async getLinkedIds(fragmentId, agentId = "default") {
    return this.links.getLinkedIds(fragmentId, agentId);
  }

  async isReachable(startId, targetId, agentId = "default") {
    return this.links.isReachable(startId, targetId, agentId);
  }
  ...
  async decayImportance() {
    return this.gc.decayImportance();
  }

  async transitionTTL() {
    return this.gc.transitionTTL();
  }
  ...
  async getRCAChain(startId, agentId = "default") {
    return this.links.getRCAChain(startId, agentId);
  }
