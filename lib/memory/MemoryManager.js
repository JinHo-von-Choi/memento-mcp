...
import { pushToQueue, pushToQueuePriority } from "../redis.js";
...
export class MemoryManager {
  ...
  async remember(params) {
     ...
     */
    if (keyId) {
      const quotaPool = getPrimaryPool();
      if (quotaPool) {
        ...
        try {
          ...
          if (keyRow && keyRow.fragment_limit !== null) {
            ...
            if (countRow.count >= keyRow.fragment_limit) {
              ...
              return {
                success : false,
                error   : "fragment_limit_exceeded",
                message : `Fragment limit reached (${countRow.count}/${keyRow.fragment_limit}). Delete unused fragments or request a higher limit.`,
                current : countRow.count,
                limit   : keyRow.fragment_limit
              };
            }
          }
    ...
    morphemeIndex.getOrRegisterEmbeddings(
      await morphemeIndex.tokenize(fragment.content).catch(() => [])
    ).catch((err) => { logWarn(`[MemoryManager] morpheme registration failed: ${err.message}`); });
    ...
    if (fragment.linked_to && fragment.linked_to.length > 0) {
      for (const linkId of fragment.linked_to) {
        await this.store.createLink(id, linkId, "related", agentId)
          .catch((err) => { logWarn(`[MemoryManager] link creation failed: ${err.message}`); });
      }
    }
    ...
    /** assertion 일관성 검사 (비동기 fire-and-forget — 레이턴시 무관) */
    this.conflictResolver
      .checkAssertionConsistency(
        { ...fragment, id, created_at: fragment.created_at ?? new Date().toISOString() },
        agentId,
        keyId
      )
      .then(({ assertionStatus }) => {
        if (assertionStatus !== "observed") {
          this.store.patchAssertion(id, assertionStatus, keyId)
            .catch(err => logWarn(`[MemoryManager] patchAssertion failed: ${err.message}`));
        }
      })
      .catch(err => logWarn(`[MemoryManager] checkAssertionConsistency failed: ${err.message}`));
    ...
    this.temporalLinker.linkTemporalNeighbors(
      ...
      logWarn(`[MemoryManager] temporalLinker failed: ${err.message}`);
    });
   ...
   *
   * 단일 PostgreSQL 트랜잭션으로 다수 파편을 INSERT한다.
   * 개별 remember() 대비 DB 라운드트립을 대폭 절감.
   *
   * @param {Object} params
   *   - fragments {Array<Object>} 파편 배열 (각각 content, topic, type, importance?, keywords?)
   *   - agentId   {string}       에이전트 ID (선택)
  ...
  async reflect(params) {
    ...
    const _insertAll = async (items) => {
      ...
      for (const r of settled) {
        if (r.status === "fulfilled") {
          ...
          logWarn(`[MemoryManager] reflect insert failed: ${r.reason?.message}`);
        }
      }
    };
    ...
    if (params.task_effectiveness) {
      try {
        ...
        logWarn(`[MemoryManager] task_feedback save failed: ${err.message}`);
    ...
    for (const f of fragments) {
      if (f.id) {
        await pushToQueuePriority(queueName, { fragmentId: f.id }).catch((err) => { logWarn(`[MemoryManager] reflect embedding queue push failed: ${err.message}`); });
      }
    }
    ...
    Promise.all(
      fragments.map(f =>
        morphemeIndex.tokenize(f.content)
          ...
          .catch((err) => { logWarn(`[MemoryManager] reflect morpheme registration failed: ${err.message}`); })
      )
    ).catch((err) => { logWarn(`[MemoryManager] reflect morpheme registration failed: ${err.message}`); });

    /** 7. narrative_summary → episode 파편 (세션 서사 요약) */
    if (params.narrative_summary) {
      const sessionId = params.sessionId || "unknown";
      await this.remember({
        content        : params.narrative_summary,
        type           : "episode",
        topic          : "session_reflect",
  ...
  async context(params) {
    ...
    try {
      ...
      logWarn(`[MemoryManager] anchor load failed: ${err.message}`);
    }
