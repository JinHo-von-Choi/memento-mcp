...
import { LATEST_PROTOCOL_VERSION } from "@modelcontextprotocol/sdk/shared/version.js";
...
function negotiateProtocolVersion(clientVersion) {
  ...
  // 2. 공식 SDK의 LATEST_PROTOCOL_VERSION과 일치하거나 그 이하인 최신 버전을 요청한 경우
  // 향후 프로토콜 마이너 업데이트 대응을 위해 클라이언트 버전을 그대로 수용 (호환성 확보)
  try {
    const clientDate = new Date(clientVersion);
    const latestDate = new Date(LATEST_PROTOCOL_VERSION);
    if (!isNaN(clientDate.getTime()) && !isNaN(latestDate.getTime()) && clientDate <= latestDate) {
      console.log(`[Protocol] Client requested ${clientVersion}, which is <= SDK latest (${LATEST_PROTOCOL_VERSION}) - accepting for forward compatibility`);
      return clientVersion;
    }
  } catch (err) {
    // 날짜 파싱 실패 시 무시하고 기존 폴백 로직 수행
  }

  // 3. 서버가 지원하지 않거나 미래의 버전을 요청한 경우,
  // 날짜 기반으로 가장 가까운 하위 버전을 선택 (Fallback)
  const clientDate         = new Date(clientVersion);
  ...
  // 4. 클라이언트 요청 버전보다 이전 버전이 없는 경우 (가장 오래된 버전 사용)
  if (!fallbackVersion) {
