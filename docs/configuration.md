...
**geminiTimeoutMs**: `config/memory.js`의 `geminiTimeoutMs` 값이 15000ms에서 **60000ms**로 상향되었다. Gemini CLI 대형 프롬프트의 레이턴시 증가에 대응한다.
...
| Preset | 설명 | 주요 제한 |
|--------|------|----------|
| `recall-only` | 읽기 전용. 쓰기 도구 차단 | remember, forget, amend, reflect 불가 |
| `write-only` | 쓰기 전용. 검색 도구 차단 | recall, context 불가 |
| `onboarding` | 신규 사용자 안내. get_skill_guide 우선 노출 | 제한 없음 (안내 강조) |
| `audit` | 감사/컴플라이언스. 쓰기 전체 차단 | remember, forget, amend, reflect, link 불가 |
